...
class ProgressReportGenerator:
    ...
    def _get_circular_logo_path(self):
        orig_logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "frontend", "public", "neuralaxis-logo.png"))
        if not os.path.exists(orig_logo_path):
            return None
            
        circular_logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "cache", "neuralaxis-logo-circular.png"))
        os.makedirs(os.path.dirname(circular_logo_path), exist_ok=True)
        
        # Crop only if cache does not exist or the original image is newer
        if not os.path.exists(circular_logo_path) or os.path.getmtime(orig_logo_path) > os.path.getmtime(circular_logo_path):
            from PIL import Image, ImageDraw
            try:
                img = Image.open(orig_logo_path).convert("RGBA")
                # Bounding box of the circular logo: col 138 to 888, row 383 to 1134
                ...
                cropped.save(circular_logo_path, "PNG")
            except Exception as e:
                print(f"Error generating circular logo: {e}")
                return orig_logo_path # Fallback to original
    ...
    def generate_report(self, output_path, data, insights, financials=None):
        ...
        # --- SECTION BRANDING (WATERMARK/HEADER) ---
        ...
        doc.add_paragraph("Prepared by: Dennis Chomba")
    ...
    def _add_cover_page(self, doc, data):
        ...
        if logo_path:
            ...
            # Reduce top spacer since logo takes vertical space
